(function (lwc) {
    ...
    **/
    ...
    function normalizeFormattedDate(value, format) {
      ...
      const parsedDate = parseDateTime$1(dateValue, format || mediumDateFormat, true);
    ...
    */
    ...
    class LightningTimePicker extends lwc.LightningElement {
      ...
      format(date, formatString) {
        if (formatString) {
          return formatTime$1(date, formatString);
        }
    ...
    function fromIsoDecimal(numberAsString) {
      const decimalSeparator = getLocale().decimal;
    ...
    function getLocaleSeparators() {
      const decimalSeparator = getLocale().decimal;
      const groupSeparator = getLocale().grouping;
      return {
        decimalSeparator,
        groupSeparator
      };
    ...
    * */
    ...
    var TextTpl = lwc.registerTemplate(tmpl$l);
    tmpl$l.stylesheets = [];
    ...
    var PhoneTpl = lwc.registerTemplate(tmpl$m);
    tmpl$m.stylesheets = [];
    ...
    var EmailTpl = lwc.registerTemplate(tmpl$n);
    tmpl$n.stylesheets = [];
    ...
    var PercentTpl = lwc.registerTemplate(tmpl$o);
    tmpl$o.stylesheets = [];
    ...
    var UrlTpl = lwc.registerTemplate(tmpl$p);
    tmpl$p.stylesheets = [];
    ...
    var CurrencyTpl = lwc.registerTemplate(tmpl$q);
    tmpl$q.stylesheets = [];
    ...
    var NumberTpl = lwc.registerTemplate(tmpl$r);
    tmpl$r.stylesheets = [];
    ...
    var BooleanTpl = lwc.registerTemplate(tmpl$s);
    tmpl$s.stylesheets = [];
    ...
    var DateLocalTpl = lwc.registerTemplate(tmpl$t);
    tmpl$t.stylesheets = [];
    ...
    var DateTpl = lwc.registerTemplate(tmpl$u);
    tmpl$u.stylesheets = [];
    ...
    var DefaultTpl = lwc.registerTemplate(tmpl$v);
    tmpl$v.stylesheets = [];
    ...
    class LightningPrimitiveDatatableIeditTypeFactory extends lwc.LightningElement {
      ...
      set columnDef(value) {
        assert$1( // eslint-disable-next-line no-prototype-builtins
    ...
    function getRowNumberErrorColumnDef(rowErrors, rowTitle) {
      ...
      const alternativeText = formatString$1(i18n$p.rowLevelErrorAssistiveText, rowTitle || '', rowErrors.fieldNames ? rowErrors.fieldNames.length : '');
      return {
        type: COLUMN_TYPE,
        typeAttributes: {
          error: {
    ...
    function tmpl$$($api, $cmp, $slotset, $ctx) {
      ...
      return [api_custom_element("lightning-datatable", _lightningDatatable, {
        props: {
          "columns": $cmp.normalizedColumns,
          "data": $cmp.normalizedData,
          "keyField": $cmp.keyField,
          "hideCheckboxColumn": $cmp.hideCheckboxColumn,
          "maxColumnWidth": $cmp.maxColumnWidth,
          "minColumnWidth": $cmp.minColumnWidth,
          "resizeColumnDisabled": $cmp.resizeColumnDisabled,
          "rowNumberOffset": $cmp.rowNumberOffset,
          "selectedRows": $cmp.selectedRows,
          "showRowNumberColumn": $cmp.showRowNumberColumn,
          "isLoading": $cmp.isLoading
        },
        key: 0,
        on: {
          "rowselection": _m0 || ($ctx._m0 = api_bind($cmp.handleRowSelection)),
          "headeraction": _m1 || ($ctx._m1 = api_bind($cmp.handleHeaderAction)),
          "rowaction": _m2 || ($ctx._m2 = api_bind($cmp.handleRowAction))
        }
      }, [])];
    }
    ...
    tmpl$$.stylesheets = [];
    tmpl$$.stylesheetTokens = {
      hostAttribute: "lightning-treeGrid_treeGrid-host",
      shadowAttribute: "lightning-treeGrid_treeGrid"
    };

    const whitelistedColumnKeys = ['actions', 'cellAttributes', 'fieldName', 'iconLabel', 'iconName', 'iconPosition', 'initialWidth', 'label', 'type', 'typeAttributes'];
    /**
    ...
    const normalizeData = function (data, expandedRows, keyField, depth, flattenedData = []) {
      ...
      data.forEach((row, index) => {
        // create new object
        const flatRow = getObjectWithoutKeys(row, ['_children']); // add to flattened data
        ...
        if (row.hasOwnProperty('_children')) {
          ...
          flattenedData.push(flatRow); // validate _children key
          ...
          if (flatRow.isExpanded && hasChildrenContent) {
            normalizeData(row._children, expandedRows, keyField, depth, flattenedData); // if we have a row being expanded but we don't have children content don't expand it
    ...
    const normalizeColumns$1 = function (columns) {
      const normalizedColumns = []; // generate a new column object to replace the first column passed in

      if (Array.isArray(columns) && columns.length > 0) {
        const treeColumn = getObjectWithoutKeys(getSanitizedObject(columns[0], whitelistedColumnKeys), ['typeAttributes']);
        treeColumn.typeAttributes = {}; // move current typeAttributes to typeAttributes.subTypeAttributes

        if (columns[0].typeAttributes) {
          treeColumn.typeAttributes.subTypeAttributes = columns[0].typeAttributes || {};
        } // transform the first column definition
        // add needed type attributes for the tree type


        treeColumn.typeAttributes.level = {
          fieldName: 'level'
        };
        treeColumn.typeAttributes.setSize = {
          fieldName: 'setSize'
        };
        treeColumn.typeAttributes.posInSet = {
          fieldName: 'posInSet'
        };
        treeColumn.typeAttributes.hasChildren = {
          fieldName: 'hasChildren'
        };
        treeColumn.typeAttributes.isExpanded = {
          fieldName: 'isExpanded'
        };
        treeColumn.typeAttributes.subType = columns[0].type; // change type to tree

        treeColumn.type = 'tree'; // replace column with proper tree version

        normalizedColumns[0] = treeColumn; // iterate over remaining columns and remove any unsupported settings/keys
    ...
    function getObjectWithoutKeys(object, ignoredKeys) {
      const newObj = {};
      const objKeys = Object.keys(object); // iterate over the object's keys

      objKeys.forEach(key => {
        // if we hit a key listed in the ignore list, skip it
        if (ignoredKeys.includes(key)) ; else {
          newObj[key] = object[key];
        }
      });
      return newObj;
    ...
    class LightningTreeGrid extends lwc.LightningElement {
      ...
      constructor() {
        super();
        this._rawColumns = void 0;
        this._rawData = void 0;
        this._keyField = void 0;
        this._publicExpandedRows = [];
        this.hideCheckboxColumn = false;
        this.isLoading = false;
        this.maxColumnWidth = 1000;
        ...
        this.template.addEventListener('privatetogglecell', this.handleToggle.bind(this)); // event received by the tree cell type

        this.template.addEventListener('toggleallheader', this.handleToggleAll.bind(this)); // event received by the tree column header
      }

      set columns(value) {
        this._rawColumns = value;
        this._columns = normalizeColumns$1(value);
      }
      /**
       * Array of the columns object that's used to define the data types.
       * Required properties include 'label', 'fieldName', and 'type'. The default type is 'text'.
      ...
      handleToggle(event) {
        ...
        const {
          name,
          nextState
        } = event.detail; // toggle row in user provided data
      ...
      handleToggleAll(event) {
        ...
        const {
          nextState
        } = event.detail; // toggle all rows in user provided data

        this.toggleAllRows(this.data, nextState);
      }

      handleRowSelection(event) {
        event.stopPropagation(); // pass the event through
      ...
      updateExpandedRows(name, isExpanded) {
        ...
        const itemPosition = this._expandedRows.indexOf(name); // :: if it is and isExpanded is false, remove it


        if (itemPosition > -1 && isExpanded === false) {
          this._expandedRows.splice(itemPosition, 1); // :: if it is not and isExpanded is true, add it
      ...
      hasChildrenContent(row) {
        ...
        if ( // eslint-disable-next-line no-prototype-builtins
        row.hasOwnProperty('_children') && Array.isArray(row._children) && row._children.length > 0) {
          hasChildrenContent = true;
        }
      ...
      toggleRow(data, name, isExpanded) {
        ...
        data.forEach(row => {
          const hasChildrenContent = this.hasChildrenContent(row); // if we find the matching row apply the changes and trigger the collapseChange event

          if (row[this.keyField] === name) {
            this.updateExpandedRows(name, isExpanded); // fire the collapseChange event

            this.fireRowToggleChange(name, isExpanded, hasChildrenContent, row); // if we didn't find the matching node and this node has children then continue deeper into the tree
      ...
      toggleAllRows(data, isExpanded, rowsToToggle = []) {
        ...
        if (isExpanded) {
          // step through the array using recursion until we find the correct row to update
          data.forEach(row => {
            const hasChildrenContent = this.hasChildrenContent(row); // if row has children content then expand it

            if (hasChildrenContent) {
              rowsToToggle.push(row[this.keyField]); // continue deeper into the tree if we have valid children content
        ...
        if (--this._toggleAllRecursionCounter === 0) {
          this._toggleAllRecursionCounter = 1; // update the expandedRows value with all valid values

          this._expandedRows = rowsToToggle; // fire the toggleAllChange event

          this.fireToggleAllChange(isExpanded); // update the data
    ...
    lwc.registerDecorators(LightningTreeGrid, {
      publicProps: {
        hideCheckboxColumn: {
          config: 0
        },
      ...
      publicMethods: ["getSelectedRows", "getCurrentExpandedRows", "expandAll", "collapseAll"],
      track: {
        _columns: 1,
        _data: 1,
        _expandedRows: 1
      },
      fields: ["_rawColumns", "_rawData", "_keyField", "_publicExpandedRows", "_toggleAllRecursionCounter"]
    });

    var _lightningTreeGrid = lwc.registerComponent(LightningTreeGrid, {
      tmpl: _tmpl$I
    });
    ...
    function tmpl$10($api, $cmp, $slotset, $ctx) {
      ...
      return [api_element("div", {
        ...
        key: 0
      }, [api_text("Tree Grid component with data from a static source. Click on the arrow icons to expand or collapse a branch.")]), api_element("div", {
        classMap: {
          "slds-p-around_medium": true,
          "lgc-bg": true
        },
        key: 2
      }, [api_custom_element("lightning-tree-grid", _lightningTreeGrid, {
        props: {
          "columns": $cmp.gridColumns,
          "data": $cmp.gridData,
          "keyField": "name"
        },
        key: 1
    ...
    class TreeGridBasic extends lwc.LightningElement {
      constructor(...args) {
        super(...args);
    ...
    lwc.registerDecorators(TreeGridBasic, {
      fields: ["gridColumns", "gridData"]
    });

    var main = lwc.registerComponent(TreeGridBasic, {
      tmpl: _tmpl$J
    });
