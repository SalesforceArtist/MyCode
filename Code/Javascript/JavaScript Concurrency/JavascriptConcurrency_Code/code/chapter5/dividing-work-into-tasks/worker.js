...
addEventListener('message', (e) => {
	...
	// We need to store refernces to our sub-workers,
	// so we can terminate them.
	var workers = [];

	// This is for counding the number of results
	...
	for (let chunk of chunks) {
		...
		// Launches our sub-worker and stores a reference
		// to it in "workers".
		...
		worker.addEventListener('message', (e) => {
			...
			// The the result is "truthy", we can post a
			// response back to the main thread. Otherwise, we
			// check if all the responses are back yet. If so,
			// we can post a false value back. Either way, we
